...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        # if np.random.uniform() > 0.5:
        #     for i in range(1, len(tokens)):
        #         if labels[i] == 'I-NAME_STUDENT' and np.random.uniform(0, 1) > 0.9:
        #             tokens[i] = '\n'
        #             labels[i] = 'O'
        ...
        if np.random.uniform() > 0.5:
            for i in range(2, len(tokens)):
                if labels[i] == 'I-NAME_STUDENT' and np.random.uniform(0, 1) > 0.9:
                    tokens = tokens[:i+1] + [' ', tokens[i-1], tokens[i], ' '] + tokens[i+1:]
                    labels = labels[:i+1] + [' ', labels[i-1], labels[i], ' '] + labels[i+1:]
...
if __name__ == '__main__':
    ...
    config_fp = 'configs/exp076_config.yaml'
